    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql("SELECT * FROM user", [], function (tx, rs) {
            if (rs.rows.length == 0) {
                ...
                user.innerHTML = rs.rows.item(0).name + " 已选购一下商品";
...
function changBookNum(id) {
    ...
    var stop = user.indexOf(' ');
    var name = user.substring(0, stop);
    self.location = "addShoppingCart?id=" + id + "&username=" + name + "&number=" + number;
}
...
function jiesuan() {
    ...
    var stop = user.indexOf(' ');
    var name = user.substring(0, stop);
    self.location = "confirmOrder?username=" + name;
}
